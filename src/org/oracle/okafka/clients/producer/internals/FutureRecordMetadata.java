...
public final class FutureRecordMetadata implements Future<RecordMetadata> {
    ...
    RecordMetadata value() {
        ...
        if(this.result.msgIds() != null) {
        	try {
        		// Changes for 2.8.1
        		if (this.result.msgIds() != null && this.result.msgIds().size() > relativeOffset)
        		{
        			String msgId = this.result.msgIds().get((int)relativeOffset); 
        			baseOffset = result.baseOffset();
        			relOffset = MessageIdConverter.getRelativeOffset(msgId);
        		}
