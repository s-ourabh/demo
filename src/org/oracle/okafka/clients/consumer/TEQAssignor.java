...
public class TEQAssignor extends AbstractPartitionAssignor {
	...
	/*
	 * With Oracle Transactional Event Queue (TEQ) and in Oracle RAC environment, different partitions of a Topic can be owned
	 ...
	 * 
	 * This TEQAssignor's assign method tries to distribute partitions so that, preference is given to Local assignment.
	 * Thus assign method follows below logic for assigning partitions.
	 * 
	 * 1. Equal and fair distribution of partitions among alive sessions
			...
			Map<String, Subscription> subscriptions,  int oldVersion)
	{
		...
		//System.out.println("TEQAssignor 1:Creating assignment map for each member ");
		for (String memberId : subscriptions.keySet())
		{
			//System.out.println("TEQAssignor 2:MemberID in Assignment Map " + memberId);
		...
		for(String topicNow : partitionsPerTopic.keySet())
		{
			...
			//System.out.println("TEQAssignor: Total members to be assigned partitions are " + membersCnt);
			...
			for(Integer instNow : instPListMap.keySet())
			{
				...
				{
					//System.out.println("TEQAssignor: Assigning Partition Now from Instance " + pNow + " , " + instNow);
					...
					if(tMemNow != null)
					{
						...
						if(tMemNow.getInstanceId() == instNow && ( (tMemNow.pendingCnt > 0) || ( tMemNow.pendingCnt == 0 && !tMemNow.oneMore && floatingPartitions > 0 ) ))
						{
							...
							/*System.out.println("TEQAssignor: Partition " + pNow +" Assigned to (" + tMemNow.getSessionId() + ", " + tMemNow.getInstanceId()+"). Pending "
	...
	public Map<String, List<TopicPartition>> assign(Map<String, Integer> partitionsPerTopic,
		...
		//System.out.println("TEQAssignor: TEQ Assign 1:Creating assignment map for each member ");
		for (String memberId : subscriptions.keySet())
		{
			//System.out.println("TEQAssignor 2:MemberID in Assignment Map " + memberId);
			assignment.put(memberId, new ArrayList<TopicPartition>());
		...
		for(String topicNow : partitionsPerTopic.keySet())
		{
			...
			//System.out.println("TEQAssignor 2.1: Total members to be assigned partitions are " + membersCnt);
			//int partitionCnt = partitionsPerTopic.get(topicNow);

			//Consider total partitions actually created instead of partitions which are supposed to be created
			// TEQ creates partitions as and when message is published into the topic partitions.
			int partitionCnt = totalPartitionsCreated;
			
			int partForAll = partitionCnt/membersCnt;
			int partForSome = partitionCnt%membersCnt;
			//System.out.println("TEQAssignor 2.2: Partitions for All "  + partForAll + " Partitions for Some " + partForSome);
			...
			if(partForSome > 0 )
			{
				...
				for(SessionData sDataNow : memberList)
				{
					...
					instSDataList.add(sDataNow);
					//System.out.println("TEQAssignor: Session count for inst " + instNow + " = " + instSDataList.size());
				...
				for(int instNow : instPListMap.keySet())
				{
					...
					//System.out.println("TEQAssignor: Extra for Inst " + instNow + "= [" + extraPerInst[instNow-1] + "," + extraPerInstMax[instNow-1]+"]");
				...
				/*System.out.println("TEQAssignor: Actual Extra per Partition ");
				for(int i =0 ; i<maxInst ; i++ )
				{
					System.out.println("TEQAssignor: Inst: " + (i+1) + " Min Extra " + extraPerInst[i] +" Additional Extra " + extraPerInstMax[i] );
				}
			    // Assign one additional partition to a session 
				System.out.println("TEQAssignor: Before actual assignment, Final pending partition count per session:"); */
				for(SessionData tMem : memberList)
				{
					...
					//System.out.println("TEQAssignor: Session " + tMem.name + " To be assigned " + tMem.pendingCnt);
				}
			}

			// Logic to actual assignment of partition to session based on pendingCnt
			Iterator<SessionData> assigner = memberList.iterator();
			//System.out.println("TEQAssignor: TEQ Assign 3:Local Assignment");
			// Phase 1: Sticky-Local Assignment
			while(assigner.hasNext())
			{
				SessionData tMem = assigner.next();
				//System.out.println("TEQAssignor: Member " + tMem.name + " Pending partitions " + tMem.pendingCnt);
			...
			//System.out.println("TEQAssignor 4:Local New Assignment");
			...
			while(assigner.hasNext())
			{
				...
				for(Integer pNow : localPartitionList)
				{
					...
					//System.out.println("TEQAssignor: Partitoin " +pNow + " assigned to " + tMem.name +" Pending " + tMem.pendingCnt);
				}
			...
			//System.out.println("TEQAssignor: TEQ Assign 5:Sticky Remote Assignment");
			while(assigner.hasNext())
			{
				SessionData tMem = assigner.next();
				//System.out.println("TEQAssignor: Member " + tMem.name + " Pending partitions " + tMem.pendingCnt);
			...
			//System.out.println("TEQAssignor: TEQ Assign 6:Remote New Assignment");
			while(assigner.hasNext())
			{
				SessionData tMem = assigner.next();
				//System.out.println("TEQAssignor: Member " + tMem.name + " Pending partitions " + tMem.pendingCnt);
			...
			//System.out.println("TEQAssignor: Final Partition Map ");
			while(assigner.hasNext())
			{
				SessionData tMem = assigner.next();
				//System.out.println("TEQAssignor: Session: " + tMem.name );
				List<PartitionData> previousPartitions = tMem.getPreviousPartitions();
				...
				if(assignedPartitionList == null || assignedPartitionList.size() == 0)
				{
					...
					//System.out.println("TEQAssignor:        " + teqP.getTopicPartition().partition());
				...
				for(PartitionData teqPNow : assignedPartitionList)
				{
					...
					//System.out.println("TEQAssignor:        " + teqPNow.getTopicPartition().partition());
				}
				previousPartitions.clear();
